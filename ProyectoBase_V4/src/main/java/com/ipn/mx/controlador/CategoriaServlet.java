...
import hibernateDAO.CategoriaDAOH;
...
//        response.setCharacterEncoding("utf-8");
    ...
    private void actualizarCategoria(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Categoria regEncontrado = new Categoria();
        dto.setIdCategoria(Integer.parseInt(request.getParameter("id")));  //Se fija el valor del parámetro accion, mandado desde "listaDeCategorías.jsp" línea 65 y SE CASTEA, por que el valor del parámetro se recibe como un STRING  
...
//            dao.delete(regEncontrado);
    ...
    private void almacenarCategoria(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (variable.isEmpty()) {       //Significa que es un registro nuevo
            ...
            dto.setNombreCategoria(request.getParameter("txtNombre"));
            dto.setDescripcionCategoria(request.getParameter("txtDescripcion"));
            dto.setIdCategoria(Integer.parseInt(variable)); //Se recupera el id con "variable"
            dao.update(dto);
            listadoCategorias(request, response);
